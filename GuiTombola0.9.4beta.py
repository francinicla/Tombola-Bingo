...
class Menub(object):
    ...
    def __init__(self, window):
        ...
        def maboutinfo():
            messagebox.showinfo(title="About", message="Gioco della Tombola\n\nCreated by\n\nClaudio Francini")
...
class Board:
    ...
    def show_board(self):
        ...
        def configu():
            ...
            Board.player_num = num_player
            Board.montepremi[0] = premi
        ...
        Label(text='Inserire quanti punti premio vuoi giocare').pack()
        premio1 = Entry(tconf, textvariable=punti)
        premio1.insert(END, "")
        premio1.pack()

        Button(tconf, text='COMINCIAMO!!', command=configu).pack(side=BOTTOM,pady=30)
        ...
        radio_card.pack(side=LEFT, padx=30, pady=10)
...
class Game:
    ...
    def __init__(self):
        ...
        Label(premio, text='Cinquina = %2d' % (mpremi * 27.5 / 100), font="Helvetica 10").grid(padx=10)
        Label(premio, text='Tombola = %2d' % (mpremi * 35 / 100), font="Helvetica 10").grid(padx=10)
        ...
        for pos_gio in range(play_cards[0]):
            Label(cards, text='Nome giocatore:%s  premi:%d' % (name_play[pos_gio], Board.montepremi[pos_gio +1])).grid(row=0 + posy, column=1 + posx,
                                                                               columnspan=10)
            posy += 1
        ...
        def vincita(numero):
            ...
            for i in elen_tuple:
                ...
                for k in i:
                    ...
                    if Board.win == 0:
                        if n == 2:
                            Board.montepremi[gi+1] += Board.montepremi[0]*5/100
                            # print(Board.montepremi)
                            Board.win = 1
                            messagebox.showinfo(title="AMBO!!!",
                                                        message="\nIL GIOCATORE %s \nCON LA CARTELLA %s \nHA FATTO "
                                                                "\nAMBO!!!!!!"
                    ...
                    if Board.win == 1:
                        if n == 3:
                            Board.montepremi[gi+1] += Board.montepremi[0]*12.5/100
                            # print(Board.montepremi)
                            Board.win = 2
                            messagebox.showinfo(title="TERNA!!!",
                                                        message="\nIL GIOCATORE %s \nCON LA CARTELLA %s \nHA FATTO "
                                                                "\nTERNA!!!!!!"
                                                                % (Board.name_players[gi],
                                                                   cartel % Board.players_cards[1] + 1))
                    ...
                    if Board.win == 2:
                        if n == 4:
                            Board.montepremi[gi + 1] += Board.montepremi[0] * 20 / 100
                            # print(Board.montepremi)
                            Board.win = 3
                            messagebox.showinfo(title="QUATERNA!!!",
                                                        message="\nIL GIOCATORE %s \nCON LA CARTELLA %s \nHA FATTO "
                                                                "\nQUATERNA!!!!!!"
                                                                % (Board.name_players[gi],
                                                                   cartel % Board.players_cards[1] + 1))
                    ...
                    if Board.win == 3:
                        if n == 5:
                            Board.montepremi[gi+1] += Board.montepremi[0]*27.5/100
                            # print(Board.montepremi)
                            Board.win = 4
                            messagebox.showinfo(title="CINQUINA!!!",
                                                        message="\nIL GIOCATORE %s \nCON LA CARTELLA %s \nHA FATTO "
                                                                "\nCINQUINA!!!!!!"
                                                                % (Board.name_players[gi],
                                                                   cartel % Board.players_cards[1] + 1))

                    if tombola == 15 and Board.win == 4:
                        Board.montepremi[gi+1] += Board.montepremi[0]*35/100
                        # print(Board.montepremi)
                        Board.win = 5
                        print("Elenco premi vinti")
                        for p in range(Board.player_num):
                            print("Il giocatore %s ha totalizzato %s punti" % (Board.name_players[p],
                                                                               Board.montepremi[p + 1]))

                        messagebox.showinfo(title="TOMBOLA!!!",
                                                    message="\nIL GIOCATORE %s \nCON LA CARTELLA %s \nHA FATTO "
                                                            "\nTOMBOLA!!!!!!"
                                                            % (Board.name_players[gi],
                                                               cartel % Board.players_cards[1] + 1))
...
class Cartelle:
    ...
    def genera_cartella(self):
        ...
        """Generazione colonne in ordine crescente"""
            ...
            if trovati > 1:
                if trovati == 2:
                    if tupla_cartella[primo[0][0]][primo[0][1]] > tupla_cartella[primo[1][0]][primo[1][1]]:
                        ...
                        tupla_cartella[primo[0][0]][primo[0][1]] = tupla_cartella[primo[1][0]][primo[1][1]]
                ...
                if trovati == 3:
                    ...
                    if tupla_cartella[primo[1][0]][primo[1][1]] > tupla_cartella[primo[2][0]][primo[2][1]]:
                        ...
                        tupla_cartella[primo[1][0]][primo[1][1]] = tupla_cartella[primo[2][0]][primo[2][1]]
